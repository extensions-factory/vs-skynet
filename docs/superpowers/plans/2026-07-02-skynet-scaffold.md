...
Run `pnpm --version` to get the installed pnpm version (e.g. `9.12.0`), then open `package.json` and ensure these fields are set exactly (substituting the real version for `<pnpm-version>`):
```json
{
  "name": "skynet",
  "displayName": "Skynet",
...
```
(Leave `engines`, `main`, `contributes`, and `activationEvents` as generated — they're not covered by this plan. The `packageManager` field pins the pnpm version for Corepack and `pnpm/action-setup` in CI — Task 9.)
...
- [ ] **Step 6: Verify no build artifacts would be committed, then commit**

Run: `git status --porcelain | grep -E '^\?\? (node_modules|out)/'`
Expected: no output. If either directory is listed as untracked, the generator did not produce a `.gitignore` covering it — create a minimal one now so Step 7 below doesn't commit it:
```bash
printf 'node_modules/\nout/\n' > .gitignore
```
(This is superseded by the complete version Task 8 writes later — this is only a safety net so `git add -A` below can't accidentally commit build output.)
...
```
(Remove any old plain `"test": "vscode-test"` and any generator-added `"pretest"` entry — `pretest` only auto-fires for the exact `test` script name, not for `test:unit`/`test:integration` invoked directly, and would force an unnecessary compile before every unit-test run. Making `test:integration` self-contained instead is unambiguous regardless of how it's invoked.)
...
Open the generated `biome.json`. Add the compiled output and downloaded-VSCode-instance directories to whichever exclusion mechanism its schema uses (matching what `.gitignore` covers — Task 8):
- If the file has a `"files": { "ignore": [...] }` key (Biome 1.x schema): add `"out/**"` and `".vscode-test/**"` to that array.
- If the file has a `"files": { "includes": [...] }` key with `!`-prefixed negation globs (Biome 2.x schema): add `"!out/**"` and `"!.vscode-test/**"` to that array.
...
**Files:**
- Create: `commitlint.config.cjs`, `.githooks/commit-msg`
...
Create `commitlint.config.cjs` (the `.cjs` extension forces CommonJS regardless of whether `package.json` sets `"type": "module"` — check that field now; if it's absent, a plain `commitlint.config.js` would also work, but `.cjs` is used here so this step doesn't depend on that check):
```js
module.exports = {
...
```
Expected: commit is REJECTED with a commitlint error (e.g. `subject may not be empty` / `type may not be empty`). `git add -A` (not a narrower add) is required here because Step 1 already modified `package.json` and `pnpm-lock.yaml` — those changes must be staged now so the real commit in Step 6 actually includes the new devDependencies, otherwise the lockfile would be committed out of sync with `package.json`.
...
```
Expected: commit SUCCEEDS (the message itself is a valid Conventional Commit, proving the hook both rejects bad messages and accepts good ones), and includes `package.json`/`pnpm-lock.yaml` alongside `commitlint.config.cjs` and `.githooks/` (everything staged in Step 5).
...
**Files:**
- Create or overwrite: `.gitignore` (the generator may have already created one, and Task 1 Step 6 may have written a minimal safety-net version — this replaces either with the complete version)
...
pnpm run test:unit
```
Expected: all four exit 0. (Actual CI execution is verified once this repo has a GitHub remote and this workflow runs there — out of scope for this plan.)
